...
public class BeanShellStaticDataNormalizeProcessor implements
	...
	private static Logger logger = Logger
			.getLogger(BeanShellStaticDataNormalizeProcessor.class);
	...
	private StringBuffer buildScript(List<NormalizeScriptConfigBean> _scripts)
		...
		for (NormalizeScriptConfigBean config : _scripts) {
			if ((config.getScript() != null)
				...
				logger.info(" config.getScript " + config.getScript() + " "
						+ config.getTableFieldPath());
	...
	private Interpreter prepareContext(Session _session, SDMRow row,
		...
		for (SDMValue sdmValue : row.getSdmValues()) {
			...
			if ((sdmValue.getValue() != null)
				...
				if (sdmValue.getJobField() == null) {
					...
					logger.info("El valor " + sdmValue.getId() + " del row "
		...
		if (fields.size() > 0) {
			...
			logger.info("Row " + row.getId() + " value: " + row.getValue()
					+ " No tiene fields");
		}
	...
	protected NormalizeValueResultBean scriptsProcessing(Session _session,
	...
	public void normalize(Session _session, StatelessSession statelessSession,
			String _user, String _locale) throws Exception {
		...
		System.out.println("Rows parsed to Normalize: " + sdmRows.size());
		logger.info("Rows parsed to Normalize: " + sdmRows.size());
		...
		if (!sdmRows.isEmpty()) {
			...
			for (SDMStaticRow row : sdmRows) {
				try {
					...
					logger.info("row " + row.getSecurityIsin() + " "
							+ row.getSecurityName() + " " + row.getValue());
					...
					if ((scripts != null) && (scripts.size() > 0)) {
						...
						if (result != null) {
							...
							//TODO VER SI HAY QUE MODIFICAR ESTA ASIGNACIÓN;
					...
					System.err.println("Error normalizando row " + row.getId()
							+ " " + e.getMessage());
					logger.error("Error normalizando row " + row.getId() + " "
							+ e.getMessage());
				}
			}
